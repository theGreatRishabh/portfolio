...
const NavTile: React.FC<NavTileProps> = ({ item, onClick }) => (
    ...
    >
        <div className={`absolute inset-0 ${item.color} opacity-80 group-hover:opacity-90 transition-opacity duration-300`}></div>
        <div className="relative z-10 flex flex-col items-center justify-center transform group-hover:scale-110 transition-transform duration-300">
            {item.icon}
            <span className="text-xl font-semibold tracking-wider uppercase">{item.title}</span>
        </div>
    </button>
);
...
const HomePage: React.FC<HomePageProps> = ({ setActiveSection, isExiting }) => {
    ...
    const baseTransition = 'transform transition-transform duration-500 ease-in-out';
    ...
    const leftPanelClasses = `
        ...
        ${baseTransition}
        ${isMounted && !isExiting ? 'translate-y-0 translate-x-0' : '-translate-y-full md:translate-y-0 md:-translate-x-full'}
    `;
    ...
    return (
        <div className="flex flex-col md:flex-row h-screen w-screen overflow-hidden fixed inset-0">
            ...
            <div className={rightPanelClasses}>
                {NAV_ITEMS.map(item => (
                    <NavTile key={item.id} item={item} onClick={() => setActiveSection(item.id)} />
                ))}
            </div>
        </div>
    );
